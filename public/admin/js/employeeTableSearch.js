...
document.addEventListener('DOMContentLoaded', function() {
    ...
    searchInput.addEventListener('input', function(e) {
        ...
        tableRows.forEach(row => {
            ...
            // Get all text content from the row (updated indices after column removal and reordering)
            const employeeId = row.cells[1]?.textContent.toLowerCase() || '';
            const lastName = row.cells[2]?.textContent.toLowerCase() || '';
            const firstName = row.cells[3]?.textContent.toLowerCase() || '';
            const middleName = row.cells[4]?.textContent.toLowerCase() || '';
            const sex = row.cells[5]?.textContent.toLowerCase() || '';
            const birthdate = row.cells[6]?.textContent.toLowerCase() || '';
            const age = row.cells[7]?.textContent.toLowerCase() || '';
            const status = row.cells[8]?.textContent.toLowerCase() || '';
            const designation = row.cells[9]?.textContent.toLowerCase() || '';
            const division = row.cells[10]?.textContent.toLowerCase() || '';
            const department = row.cells[11]?.textContent.toLowerCase() || '';
            
            // Combine all searchable fields
            const fullName = `${firstName} ${middleName} ${lastName}`.trim();
            const searchableText = `${fullName} ${lastName} ${firstName} ${middleName} ${sex} ${birthdate} ${age} ${employeeId} ${status} ${designation} ${division} ${department}`;
